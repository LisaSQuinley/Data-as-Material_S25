...
const search =  `botanical AND study AND unit_code:"SAAM" online_media_type:"Images"`;
...
function fetchAllData(url) {
  ...
  .then(data => {
    ...
    jsonString += JSON.stringify(myBotStudies);
    console.log(myBotStudies);
  })
  .catch(error => {
    console.log(error)
  })
...
function addObject(objectData) {  
  ...
  if(objectData.content.descriptiveNonRepeating.online_media) {
    image_url = objectData.content.descriptiveNonRepeating.online_media.media;
  }
...
console.log(objectData);
  ...
  myBotStudies.push({
    ...
    url: objectData.url,
    thumbnail_link: objectData.content.descriptiveNonRepeating.online_media.media[0].thumbnail,
    })
}
